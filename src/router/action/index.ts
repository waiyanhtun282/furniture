...
export const registerAction = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      return {
        error: error.response?.data || "Sending OTP failed. Please try again.",
      };
    } else throw error;
  }
};
...
export const otpAction = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      return {
        error: error.response?.data || "Verify OTP failed. Please try again.",
      };
    } else throw error;
  }
};
...
export const favouriteAction = async ({
  ...
  try {
    ...
    if (response.status !== 200) {
      return {
        error: response.data || "Favourite Action failed. Please try again.",
      };
    }
...
export const newPasswordAction = async ({
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      return {
        error:
          error.response?.data || "Reset password failed. Please try again.",
      };
    } else throw error;
  }
};